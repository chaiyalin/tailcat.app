...
> **Release target `0.4.0-beta.1` — candidate preview, not production acceptance:** private and group files negotiate reliable native DataChannels with one coordinated DERP restart. `TAILCAT_NATIVE_FILES_ENABLED` defaults to `0`. Production remains the separately archived `0.3.0-beta.1` baseline; mobile group hosting stays off. See [native file implementation and release evidence](NATIVE_FILE_UPGRADE.md) for exact status, limitations and outstanding gates. This native preview is public and does not use Cloudflare Access. Create tag `app-v0.4.0-beta.1` only after the complete release gates and observation period pass.
